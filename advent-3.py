...
class wire():
    def __init__(self,coor):
        ...
        self.steps=[]
    def move(self,s):
        #simulates coordination on a grid
        #step count per instruction
        count=0
        if s[0]=='R':
            while count<int(s[1:]):
                count=count+1 
                self.x=self.x+1 #simulate movement
                self.steps.append(1)#add to overall step counter
                self.coor.append([self.x,self.y])#record new position
        ...
        if s[0]=='D':
            while count<int(s[1:]):
                ...
                self.coor.append([self.x,self.y])
#instantiate wires
firstwire= wire(firstcoords)
secondwire= wire(secondcoords)
for item in inp1:
    firstwire.move(item)
    #run movement of the first wire
for thing in inp2:
    secondwire.move(thing)
    
    #run second wire



for t in firstcoords:
    if t in secondcoords:
        #find interceptions
        print('pos of 1')
        print(firstcoords.index(t))
        print('pos of 2')
        print(secondcoords.index(t))
        #this make take time. add the coords together to get a result
        #smallest result is correct, need to +2 to account for interception inclusivity
